...
/**
 * Fork of Spring Batch's FlatFileItemReader. Had to modify how this behaves when the RecordSeparatorPolicy gets a
 * null line but hasn't finished a record (we don't want it to throw an error).
 */
public class CustomFlatFileItemReader<T> extends AbstractItemCountingItemStreamItemReader<T> implements
	...
	/**
	 * This is modified from how Spring Batch works. If the line is null, don't throw an error, just return the record
	 * as-is.
	 *
	 * @param line
	 * @return
		...
		while (line != null && !recordSeparatorPolicy.isEndOfRecord(record)) {
			...
			if (line == null) {
				if (StringUtils.hasText(record)) {
					// NO LONGER THROWING AN ERROR
					// A record was partially complete since it hasn't ended but the line is null
